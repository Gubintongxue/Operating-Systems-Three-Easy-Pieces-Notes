...
```
让我更详细地理解一下 p1.c 到底发生了谁谁。当它刚开始运行谁，进程输出一条 hello 
world 信息，以及自己的进程描述符（process identifier，PID）。该进程的 PID 是 29146。UNIX
...
在 `fork()` 函数调用后，子进程的执行开始。

具体来说，`fork()` 会创建一个新的子进程，子进程是父进程的一个几乎完全相同的副本。子进程从 `fork()` 调用的返回点开始执行。对于子进程来说，`fork()` 函数会返回 0，而对于父进程，`fork()` 返回的是子进程的 PID。
...
由于 `rc` 在子进程中为 0，因此这个 `if` 条件为真，所以子进程从这里开始执行。需要注意的是，由于操作系统的调度机制，子进程的具体执行顺序可能在 `fork()` 之后立即开始，也可能稍后执行，而父进程和子进程的执行顺序是不确定的。这意味着输出顺序可能会有所不同，取决于操作系统的调度。



##### 意思是子进程的fork不会执行，直接就获得返回值开始执行吗

是的，理解正确！

在调用 `fork()` 时，子进程并不会再次执行 `fork()`。而是操作系统在 `fork()` 调用时已经完成了子进程的创建工作。对于子进程来说，`fork()` 函数调用的行为是这样的：

1. 操作系统会复制父进程的地址空间，创建一个新的子进程。
2. 这个子进程会从父进程的 `fork()` 调用的地方开始执行。
3. 在子进程中，`fork()` 函数直接返回 0，表示它是子进程。

因此，子进程实际上并没有再次执行 `fork()` 函数调用的代码，而是操作系统直接为子进程提供了一个返回值 0，让子进程从 `fork()` 的返回点继续执行。

总结来说，子进程并不会重新运行整个 `fork()`，它只是从 `fork()` 返回值的那一点开始运行代码。
