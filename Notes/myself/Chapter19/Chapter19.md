...
​		注意，VPN （页号）和 PFN 同时存在于 TLB 中，因为一条地址映射可能出现在任意位置（用硬件的术语，TLB 被称为全相联的（fully-associative）缓存）。硬件并行地查找这些项，看看是否有匹配。
...
![image-20240906100237597](image/image-20240906100237597.png)

### 补充：

在 Linux 中，VPN（Virtual Page Number）和 PFN（Page Frame Number）是与内存管理相关的概念，它们分别对应虚拟内存和物理内存。以下是对这两个概念的详细解释及其联系：

#### 1. VPN（Virtual Page Number） - 虚拟页号

- **定义**：VPN 是虚拟内存地址中的一个部分，它指示一个虚拟页面的位置。操作系统使用虚拟地址空间来为每个进程提供一个独立的、连续的内存视图。
- **作用**：当程序访问内存时，它使用虚拟地址。这些虚拟地址通过地址转换过程（页表映射）被映射到实际的物理内存地址。
- **页表**：操作系统使用页表来管理虚拟内存到物理内存的映射。页表条目（PTE）包含了虚拟页号 (VPN) 和对应的物理页帧号 (PFN) 的映射。

#### 2. PFN（Page Frame Number） - 页帧号

- **定义**：PFN 是物理内存中的页帧编号。物理内存被分割成固定大小的页帧，每个页帧的大小通常为 4KB（具体大小取决于系统配置）。
- **作用**：PFN 标识了物理内存中实际的页帧位置。当一个虚拟页被映射到物理内存时，页表中的条目将 VPN 映射到相应的 PFN。
- **内存管理**：操作系统使用 PFN 来管理物理内存的使用情况，并跟踪哪些物理页帧被分配给哪些进程。

#### VPN 和 PFN 的联系

- **映射关系**：VPN 和 PFN 之间的联系通过页表建立。页表中的每个条目包含一个 VPN 和对应的 PFN。当进程访问某个虚拟地址时，操作系统通过查阅页表将该虚拟地址的 VPN 转换为相应的 PFN，从而找到对应的物理内存地址。

- **地址转换过程**：地址转换的过程是将虚拟地址分为页号（VPN）和页内偏移量，通过查找页表找到对应的 PFN，再结合偏移量得到物理地址。

  具体过程为：

  1. **虚拟地址解析**：虚拟地址被分为 VPN 和偏移量。
  2. **查找页表**：系统查找当前进程的页表，找到 VPN 对应的 PTE。
  3. **获取 PFN**：从 PTE 中读取 PFN，将其与偏移量组合，得到物理地址。

- **性能优化**：为了优化这个地址转换过程，系统使用了 TLB（Translation Lookaside Buffer），它是一个硬件缓存，用来存储最近的 VPN 到 PFN 的映射，以加速内存访问。

总结来说，VPN 和 PFN 是内存管理中虚拟地址和物理地址之间映射的关键要素，它们通过页表紧密联系在一起，共同实现了虚拟内存到物理内存的转换。
...
![image-20240906100437439](image/image-20240906100437439.png)
...
​		为了减少这种开销，一些系统增加了**硬件支持**，实现跨上下文切换的 TLB 共享。==比如有的系统在 TLB 中添加了一个地址空间标识符（Address Space Identifier，ASID）。可以把ASID 看作是进程标识符（Process Identifier，PID），但通常比 PID 位数少（PID 一般 32 位，ASID 一般是 8 位）==。
...
![image-20240906102131261](image/image-20240906102131261.png)

​		在讨论页换出到磁盘的问题时，我们将详细研究这样的策略。这里我们先简单指出几个典型的策略。一种常见的策略是替换最近最少使用（least-recently-used，LRU）的项。LRU尝试利用内存引用流中的局部性，假定最近没有用过的项，可能是好的换出候选项。另一种典型策略就是随机（random）策略，即随机选择一项换出去。这种策略很简单，并且可以避免一种极端情况。例如，一个程序循环访问 *n*+1 个页，但 TLB 大小只能存放 *n* 个页。这时之前看似“合理”的 LRU 策略就会表现得不可理喻，因为每次访问内存都会触发 TLB未命中，而随机策略在这种情况下就好很多。
...
![image-20240906102223603](image/image-20240906102223603.png)
...
![image-20240906102855926](image/image-20240906102855926.png)
...
​		然而，TLB 的局限性也显而易见。==当程序短时间内访问的页数超过 TLB 的容量时，会导致大量的 TLB 未命中，显著降低系统性能。==为了应对这种情况，现代处理器通常支持更大的页，增加 TLB 的覆盖范围。==此外，TLB 访问本身也可能成为 CPU 流水线的瓶颈，特别是在涉及物理地址索引缓存时。这些问题促使研究人员开发了各种优化技术，例如**虚拟地址索引缓存**，以改善性能。==
...
​		另一个 TLB 问题值得一提：访问 TLB 很容易成为 CPU 流水线的瓶颈，尤其是有所谓的物理地址索引缓存（physically-indexed cache）。有了这种缓存，地址转换必须发生在访问该缓存之前，这会让操作变慢。为了解决这个潜在的问题，人们研究了各种巧妙的方法，用虚拟地址直接访问缓存，从而在缓存命中时避免昂贵的地址转换步骤。像这种虚拟地址索引缓存（virtually-indexed cache）解决了一些性能问题，但也为硬件设计带来了新问题。更多细节请参考 Wiggins 的调查[W03]。